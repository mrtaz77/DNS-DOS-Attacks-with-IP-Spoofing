...
### DNS Server with Upstream Forwarder (Hybrid Mode)
```bash
# Forward unmatched queries to Google DNS (8.8.8.8)
python -m dns_server.main \
  --zone dns_server/zones/primary.zone \
  --port-udp 5353 \
  --port-tcp 5354 \
...
**How it works:**
- Queries for domains in your zone file (e.g., `example.com`) are answered authoritatively
- Queries for external domains (e.g., `google.com`, `facebook.com`) are forwarded to the upstream resolver
- NXDOMAIN responses from upstream are handled gracefully
- This enables both authoritative and recursive DNS functionality

**Alternative upstream resolvers:**
```bash
--forwarder 1.1.1.1        # Cloudflare DNS
--forwarder 9.9.9.9        # Quad9 DNS  
--forwarder 208.67.222.222 # OpenDNS
```
